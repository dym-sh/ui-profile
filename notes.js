...
customElements.define('md-notes', class extends HTMLElement {
  async connectedCallback () {
    try {
      ...
      this.innerHTML = 'Failed to load, please refresh.'
    ...
    for (let file of files) {
      ...
      const datetime = niceDate(file.stat.ctime)
...
const niceDate = timestamp =>
  { const d = new Date( timestamp ).toISOString()
    return d.substring( 0, d.indexOf( ':' ) + 3 )
            .replace( 'T', ' @ ' )
